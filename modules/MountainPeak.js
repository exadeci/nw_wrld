...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  vec2 tc = fragCoord / 1024.0 + iTime * 0.2;
  vec3 bn = vec3(hash(tc), hash(tc + 0.1), hash(tc + 0.2));
...
class MountainPeak extends BaseThreeJsModule {
  static methods = [
    {
      name: "start",
      executeOnLoad: true,
  ...
  async start({ speed = 1.0 } = {}) {
    ...
    if (!this.elem) {
      console.error("MountainPeak: Container element not available");
      return;
    }
    ...
    if (!ShaderArt) {
      console.error("MountainPeak: ShaderArt not available");
      return;
    }
    ...
    shaderArt.setAttribute("aria-label", "Mountain Peak terrain shader by TDM");
