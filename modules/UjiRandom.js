...
class UjiRandom extends ModuleBase {
  static methods = [
    {
      name: "draw",
      executeOnLoad: true,
...
@font-face {
  ...
  runUji(opts) {
    ...
    const linesHistory = new Array(maxIter + 1);
    linesHistory[0] = copyLine(line);
    const tick = 1000 / 60;
    ...
    this.intervalId = setInterval(() => {
      ...
      speedAcc += speed;
      let steps = Math.floor(speedAcc);
      if (n < maxIter) steps = Math.min(steps, maxIter - n);
      else steps = Math.min(steps, 8);
      speedAcc -= steps;
      for (let s = 0; s < steps; s++) {
        n++;
        if (n < maxIter) drawPath(line, n, audioThickness);
        line = updateLine(line, n, audioRotationDeg, audioJitter);
        linesHistory[n % (maxIter + 1)] = copyLine(line);
      }
      if (n >= maxIter) {
        ctx.clearRect(0, 0, w, h);
        self.drawBackground(ctx, opts, w, h, r);
        const start = n - maxIter + 1;
        for (let i = start; i <= n; i++) {
          const prev = (i - 1 + (maxIter + 1)) % (maxIter + 1);
          if (linesHistory[prev]) drawPath(linesHistory[prev], i, audioThickness);
        }
      }
    }, tick);
    this.show();
  ...
  setParams(options = {}) {
    ...
    const baseOpts = this.getBaseOpts();
    if (baseOpts) requestAnimationFrame(() => this.runUji(randomizeMovement(baseOpts)));
  }
